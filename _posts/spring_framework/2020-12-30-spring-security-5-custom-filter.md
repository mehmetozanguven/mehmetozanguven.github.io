...
First thing first, there is no one filter called **AuthenticationFilter**. Instead there are many filters where chain pattern is  applied. Each chain executes its responsibilities and move forward to the next chain.
...
>
> Because I will look at the request's header to implement authentication logic and because header is the specific thing in HTTP, I must convert ServletRequest to the HttpServletRequest. 
>
...
public class MyCustomAuthentication extends UsernamePasswordAuthenticationToken {
    ...
    // this constructor creates a Authentication instace which is fully authenticated
